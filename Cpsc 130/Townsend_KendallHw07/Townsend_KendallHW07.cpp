...
void ReadFile(const Customer customers[], int& count);
...
{
	int infileid;
	double infilebalance;
	string infilefirstname, infilelastname;
	...
	else
	{
		...
		while(!infile.eof() && valid)
		{
			...
			customers[count].GetID();

			customers[count].GetBalance();

			customers[count].GetLastName() +", " + customers[count].GetFirstName();

			if(infile.fail())
			{
				valid = false;
			}
			else if(customers[count].GetID() <= 0 || customers[count].GetFullName() == "")
			{
				valid = false;
			}
			else
...
void WrittenFile(ostream& out, const Customer customers[], int count)
{
	...
	{
		out << customers[i].GetID() << " " <<  customers[i].GetBalance() << " " << customers[i].GetLastName() +", " + customers[i].GetFirstName() << '\n';
	}
