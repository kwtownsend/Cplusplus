...
//  Gnu compiler: g++ -std=c++0x -c -m64 -Wall -l exhaustive.lis -o exhaustive.o exhaustive.cpp
//  Gnu linker:   g++ -m64 -o exhaustive.out exhaustive.o
...
void print_perm(int, int *, float, point2D*, int *, float &);
  ...
  for(i=0; i<n; i++)
{
  ...
  float seconds = microseconds / 1E6;
...
void print_perm(int n, int *A, float sizeA, point2D *P, int *bestSet, float &bestDist) 
// function to generate the permutation of indices of the list of points
{
  int i;
  float dist=0;
  ...
  if (n == 1) 
  {
   for( i=0; i < sizeA-1; i++) //last digit is 2 from end because it compares with i+1, and would go to garbage data if you went to last digit
  {
	dist = dist + sqrt( pow((P[A[i]].x - P[A[i+1]].x),2) + ( pow((P[A[i]].y - P[A[i+1]].y), 2) ) );
	if( i == sizeA-2) //digit that compares with the last digit
	{
		dist = dist + sqrt( pow((P[A[0]].x - P[A[i+1]].x),2) + ( pow((P[A[0]].y - P[A[i+1]].y), 2) ) );
		if(dist < bestDist)
		{
			bestDist=dist;
