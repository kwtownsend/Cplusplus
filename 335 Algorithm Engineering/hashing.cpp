...
//  Gnu compiler: g++ -c -m64 -Wall -l hashing.lis -o hashing.o hashing.cpp
//  Gnu Linker: g++ -m64 -o hashing.out hashing.o
...
// start with table T1
	...
	while((!placed ) && (counter < 2*tablesize)) 
	{
		...
		else 
		{
			...
			cout << endl;
      			//STORE IN temp THE STRING STORED AT t[pos][index] AND STORE IN t[pos][index] THE STRING temp_s
			strcpy(temp, t[pos][index]);
			strcpy(t[pos][index], temp_s);

			// NOW temp_s CONTAINING THE EVICTED STRING NEEDS TO BE STORED IN THE OTHER TABLE
			strcpy(temp_s, temp); //temp is the evicted string that was replaced
